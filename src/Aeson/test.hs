...
getDecode :: (Response BL.ByteString) -> BL.ByteString
getDecode str = 
    let dec = DA.eitherDecode (str ^. responseBody)
    in case dec of
        Left val -> BL.fromStrict (DB.pack val)
        Right err -> error err
...
getURLContent2 urlString =
    let 
        ...
        opt = defaults & header "Accept" .~ ["application/json"]
        newHead = opt & auth .~ (Just authKey)
        urlData = getWith newHead urlString
    in urlData >>= \ud -> pure (getDecode ud)