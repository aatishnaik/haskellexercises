...
formatMksheet str = 
    let 
        dec=DC.decode NoHeader (BL.fromStrict str) :: Either String (V.Vector (String,String,String))
    in case dec of
        Right val -> 
            let 
                gdec = V.map (\(s1,s2,s3)->[s1,s2,s3]) val
        ...
        case (DL.head recrd) of
            (sname:subname:mark:[]) ->
                case ((readMaybe mark) :: Maybe Integer) of
                    Just mk -> if snm == ""
                        then studEntry (DL.tail recrd) (sname,[(subname,mk)])
                        else studEntry (DL.tail recrd) (snm,slst++[(subname,mk)])
