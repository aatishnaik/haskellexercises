...
subAvg mksheet subjects subname = let subarr = (filter (\x -> (fst x == subname)) (foldl' (\arr x -> arr ++ (snd x)) [] (allValidSubjects mksheet subjects)))
...
--validSubjects :: MarkSheet -> [SubjectName] -> StudentName -> [(StudentName,SubjectMarks)]
